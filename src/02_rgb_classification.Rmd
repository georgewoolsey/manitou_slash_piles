...
terra::coltab(rast_temp) <- data.frame(
  value = 1:(terra::unique(rast_temp) %>% nrow())
  , col = viridis::turbo(terra::unique(rast_temp) %>% nrow(), alpha = 0.9)
)
# plot
rast_temp %>% 
  terra::plot(colNA = "black", axes=F)
terra::plot(
  slash_piles_polys %>% 
    sf::st_transform(terra::crs(ortho_rast)) %>% terra::vect()
...
it looks like the automatic clustering did not consistently classify the pile locations
...
### 
terra::subst(rast_temp, from = c(14,15,13,19), to = 1, others = NA) %>% terra::plot(axes = F, legend = F)
terra::plot(
  slash_piles_polys %>% 
    sf::st_transform(terra::crs(ortho_rast)) %>% terra::vect()
  , add = T, border = "blue", col = NA, lwd = 1.2, axes=F
