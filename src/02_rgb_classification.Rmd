...
terra::plot(class_rast, alpha = 0.2, add = T, axes=F, legend = F)
superpixels_slico_sf %>% terra::vect() %>% terra::plot(add = T)
```
...
we can also see what it looks like if we include only pixels with >20% (~median value) of their area covered by a pile

```{r}
training_df %>% 
  ggplot2::ggplot() +
  ggplot2::geom_sf(ggplot2::aes(fill = pr_pile>0.2)) +
  ggplot2::geom_sf(data = slash_piles_polys, fill = NA, color = "blue", lwd = 0.3) +
...
we can also see what it looks like if we include only pixels with >10% of their area covered by a pile
