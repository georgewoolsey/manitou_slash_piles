...
the panchromatic band itself doesn't add much new information, let's remove it and generate a data frame for training kmeans clustering
...
### `OpenImageR` package SLIC clustering

the `OpenImageR` [package](https://github.com/mlampros/OpenImageR) makes the Simple Linear Iterative Clustering (SLIC) clustering algorithm ([Achanta et al. 2012](https://scholar.google.com/scholar?cluster=2726314889973535954&hl=en&as_sdt=0,6)) available in R
...
let's look at the superpixels overlaid on the RGB image

```{r}
# OpenImageR::NormalizeObject = normalize a vector, matrix or array (in the range between 0 and 1)
OpenImageR::NormalizeObject(superpixels_slico_ans$slic_data) %>% 
  terra::rast() %>% 
  terra::subset(1:3) %>% 
  terra::plotRGB(stretch = "lin", colNA = "black")
...
that's neat...generally, the steps in image segmentation would be:
...
ok, let's increase the number of superpixels to get finer detail (but significantly increase processing time)
...
let's see the output based on **Superpixels + AP** and we'll add the pile outlines in blue

```{r}
superpixels_slicap_ans %>% str()
rast_temp <- ortho_rast %>% terra::subset(1)
rast_temp[] <- superpixels_slicap_ans$AP_image_data[,,1] %>% terra::rast()
rast_temp$green <- ortho_rast %>% terra::subset(1)
rast_temp$green[] <- superpixels_slicap_ans$AP_image_data[,,2] %>% terra::rast()
rast_temp$blue <- ortho_rast %>% terra::subset(1)
rast_temp$blue[] <- superpixels_slicap_ans$AP_image_data[,,2] %>% terra::rast()
# rast_temp %>% terra::crs()
