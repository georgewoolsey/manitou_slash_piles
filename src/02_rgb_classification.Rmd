...
pred_rast %>% 
  terra::pairs(
    maxcells = min(10000, terra::ncell(pred_rast)*.01)
  )
```
...
first, we'll generate a set of sample points to extract raster values from 
...
grvi_rast %>% 
  terra::mask(
    ...
    terra::spatSample(size = 10, na.rm = T)
# pile samples
  slash_piles_polys %>% 
    dplyr::filter(!is.na(comment)) %>% 
    sf::st_transform(terra::crs(grvi_rast)) %>% 
    terra::vect() %>% 
    terra::spatSample(size = 100) %>% 
    sf::st_as_sf() %>% 
    dplyr::mutate(
      is_pile = 1
    ) %>% 
