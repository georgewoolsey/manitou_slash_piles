...
now we'll make a grid of 0.5 ha to sample from to define our training versus validation areas. we'll implement simple random sampling from this 0.5 ha grid.
...
sample_grid <- ortho_rast[[1]] %>% 
  terra::clamp(lower=0,upper=0) %>% 
  terra::aggregate(fact = sqrt(10000*0.5)/terra::res(ortho_rast[[1]])[1])
...
the `OpenImageR` [package](https://github.com/mlampros/OpenImageR) makes the Simple Linear Iterative Clustering (SLIC) clustering algorithm ([Ren & Malik 2003](https://scholar.google.com/scholar?cluster=8327809513720180304&hl=en&as_sdt=0,6)) available in R
...
######
# read image()
im_temp <- OpenImageR::readImage(fpath_class)
#--------------
# "slic" method
#--------------
fpath_class
res_slic = OpenImageR::superpixels(input_image = im_temp,
                       method = "slic",
                       superpixel = 200, 
                       compactness = 20,
...
OpenImageR::NormalizeObject(res_slic$slic_data) %>% 
  terra::rast() %>% 
  terra::subset(3) %>% 
  terra::plot()
