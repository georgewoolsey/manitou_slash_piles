...
if(!dir.exists("../data/point_cloud_processing_delivery")){
  ...
  dtm_temp <- terra::rast("../data/xxpoint_cloud_processing_delivery/dtm_0.5m.tif")
  chm_temp <- terra::rast("../data/xxpoint_cloud_processing_delivery/chm_0.2m.tif")
...
filtered_watershed_ans <- watershed_ans %>% 
  terra::subst(from = keep_segs_temp, to = keep_segs_temp, others = NA)
# plot it 
filtered_watershed_ans %>% 
  terra::plot(
    col = c(
      viridis::turbo(n = floor(nrow(terra::freq(filtered_watershed_ans))/3))
      , viridis::viridis(n = floor(nrow(terra::freq(filtered_watershed_ans))/3))
      , viridis::cividis(n = floor(nrow(terra::freq(filtered_watershed_ans))/3))
    ) %>% sample()
    , legend = F
    , axes = F
  )
...
zomg...we are getting so close. now let's try to filter based on Hough circle fitting or some sort of regular shape filter to get rid of irregular shapes.
...
```{r, include = F, eval = F}
pile_id_temp <- 
  # slash_piles_polys %>% dplyr::arrange(desc(diameter)) %>% dplyr::slice(1) %>% dplyr::pull(pile_id)
  73
...
piles_bbox_temp <- piles_temp %>% sf_bbox_by_row(corner = "ul")
